...
```{r}
plot( clinic$age_at_initial_pathologic_diagnosis, clinic$"fill in weight column name", xlab = "add x axis label", ylab = "add y axis label", main = "cool and descriptive title!" )
```
...
par(mar=c(11,1,1,1))
boxplot( clinic$age_at_initial_pathologic_diagnosis ~ clinic$race_list, xlab = "Race", ylab = "Age at Diagnosis", main = "Race vs. Age at Diagnosis", las = 2, cex.axis = 0.5)
```
...
old_patient_ids <- clinic$age_at_initial_pathologic_diagnosis[ bool_old ]
```
...
```{r}
write.csv(clinic, "/Users/korriehan/Desktop/coad_clinical_data.csv")
```
